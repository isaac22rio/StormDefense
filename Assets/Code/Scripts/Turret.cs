...
public class Turret : MonoBehaviour
{
    ...
    [SerializeField] private float damage; //bullet per second\
    ...
    private void Awake() {
        onNewWave.AddListener(ResetStats);
    ...
    private void FindTarget() {
        RaycastHit2D[] hits = Physics2D.CircleCastAll(turretRotationPoint.position, targetingRange, 
        (Vector2)turretRotationPoint.position, targetingRange, enemyMask);
    ...
    private void ApplyNerfs() {
        if (LevelManager.main.weather == "Heatwave") {
            ...
            targetingRange = defaultTargetingRange / (1 + LevelManager.main.diff_Factor);

        } else if (LevelManager.main.weather == "Rainstorm") {
            damage = ((float)(defaultDamage / (1 + LevelManager.main.diff_Factor)));
        }
    }
